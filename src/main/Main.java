...
//        String inputFileName = "testcases/07.txt";
        ...
        ArrayList<Point2D> test = FindSamplePosition(ps.getObstacles(), 100);
    ...
    public static void connectMiddle(ASVConfig head, ASVConfig tail, List<Obstacle> obstacles) {
        ...
        if (maxDist <= 0.001) {
            ...
            if (Math.abs(angle) <= Math.PI) {
                ...
                temp = loopForMinimalStep((int)count, temp, delta);
                temp.setParent(tail);
            } else {
                if (angle > 0){
                    double count = maxDist / 0.0003;
